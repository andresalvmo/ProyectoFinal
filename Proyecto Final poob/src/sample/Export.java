...
public class Export implements IExport {
    ...
    @Override
    public void exportConstantemente(List<Exportable> exportable, Character separateValue) throws Exception
    {
        String fileName = String.format("export-constante.%s", Exportable.getExtension(separateValue));
        FileOutputStream out = new FileOutputStream(fileName);
        PrintWriter pw = new PrintWriter(out);
